...
WIN_SetErrorFromHRESULT(const char *prefix, HRESULT hr)
{
    ...
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, hr, 0,
                  buffer, ALmixer_arraysize(buffer), NULL);
    message = WIN_StringToUTF8(buffer);
    ALmixer_SetError("%s%s%s", prefix ? prefix : "", prefix ? ": " : "", message);
    ALmixer_free(message);
    return -1;
}
...
WIN_CoInitialize(void)
{
    /* SDL handles any threading model, so initialize with the default, which
       ...
       If you need multi-threaded mode, call CoInitializeEx() before ALmixer_Init()
    */
#ifdef __WINRT__
